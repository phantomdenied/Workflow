(function(){
  ...
  function getApiKey(){
    ...
    if(!key){
      try{
        key = localStorage.getItem('geminiKey') || localStorage.getItem('apiKey') || localStorage.getItem('anthropicKey') || '';
      }catch(e){}
    }
    if(!key){
      try{
  ...
  function renderTaskSuggestions(resultEl, payload){
    ...
    const items = tasks.map((t, idx) => `
      <div class="ai-suggest-item">
        ...
        <div class="ai-inline-row"><button class="ai-mini-btn" data-ai-add-one="${idx}">Add Task</button></div>
      </div>`).join('');
    resultEl.innerHTML = summary + (tasks.length ? `
      <div class="ai-inline-row" style="margin-bottom:8px;"><button class="ai-chip-btn" id="aiAddAllTasksBtn">Add All Tasks</button></div>
  ...
  async function noteSummarize(){
    ...
    if(resultEl){ resultEl.style.display = 'block'; resultEl.innerHTML = 'Thinking…'; }
    try{
      const data = await runJsonTask('You summarize operations notes. Return JSON only: {"summary":"2-4 sentence summary"}', txt);
      if(resultEl) resultEl.innerHTML = '<strong>Summary:</strong><div style="margin-top:6px;">' + escHtml(data.summary || '') + '</div>';
    }catch(e){
      console.error(e);
      if(resultEl) resultEl.innerHTML = escHtml(e.message);
    }
  }
  ...
  async function noteRewrite(){
    ...
    if(!txt){ toastSafe('Write or paste a note first'); return; }
    try{
      const data = await runJsonTask('You rewrite rough notes into concise, professional, plain-English operations notes. Keep all factual meaning. Return JSON only: {"text":"rewritten text"}', txt);
  ...
  async function noteMakeTasks(){
    ...
    if(!txt){ toastSafe('Write or paste a note first'); return; }
    if(resultEl){ resultEl.style.display = 'block'; resultEl.innerHTML = 'Extracting tasks…'; }
    try{
      const data = await runJsonTask('Extract actionable tasks from operations notes. Return JSON only with this exact shape: {"summary":"short summary","tasks":[{"name":"short action task","notes":"context","priority":"high|medium|low","dueDate":"YYYY-MM-DD or null","houseName":"exact house name from the list or null"}]}. Use only these house names if relevant: ' + houses, txt);
      if(resultEl) renderTaskSuggestions(resultEl, data);
    }catch(e){
      console.error(e);
      if(resultEl) resultEl.innerHTML = escHtml(e.message);
    }
  ...
  async function emailMakeTasks(){
    ...
    if(resultEl){ resultEl.style.display = 'block'; resultEl.innerHTML = 'Reading email…'; }
    try{
      const data = await runJsonTask('Extract actionable tasks from an email. Return JSON only with this exact shape: {"summary":"short summary","tasks":[{"name":"short action task","notes":"context from email","priority":"high|medium|low","dueDate":"YYYY-MM-DD or null","houseName":"exact house name from the list or null"}]}. Use only these house names if relevant: ' + houses, txt);
      if(resultEl) renderTaskSuggestions(resultEl, data);
    }catch(e){
      console.error(e);
      if(resultEl) resultEl.innerHTML = escHtml(e.message);
  ...
  async function polishField(textarea, mode){
    ...
    try{
      const data = await runJsonTask('You write concise, professional documentation comments for operations forms. Return JSON only: {"text":"final text"}', userPrompt);
      if(data.text){
  ...
  function ensureEmailCard(){
    ...
    card.innerHTML = `
      ...
      <div class="ai-inline-row"><button class="ai-chip-btn" id="aiEmailTaskBtn">📧 Create Tasks from Email</button></div>
      <div id="aiEmailResult" class="ai-result-box"></div>`;
    const filterWrap = document.getElementById('notesFilterWrap');
    if(filterWrap && filterWrap.parentNode) filterWrap.parentNode.insertBefore(card, filterWrap.nextSibling);
    else notesScreen.appendChild(card);
  ...
  function enhanceFormTextareas(rootSel){
    ...
    root.querySelectorAll('textarea').forEach((ta) => {
      ...
      row.innerHTML = `<button class="ai-mini-btn">✨ Polish</button><button class="ai-mini-btn">💡 Suggest</button>`;
