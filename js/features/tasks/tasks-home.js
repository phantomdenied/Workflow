...
/* Fallback summary helpers so Home rendering does not break if feature files load later. */
window.isDA3Overdue = window.isDA3Overdue || function(){
  const now = new Date();
  return now.getDate() > 4;
};

window.calcDA3Pct = window.calcDA3Pct || function(v){
  if(!v) return 0;
  try{
    let tot=0, done=0;
    (window.DA3||[]).forEach(s=>(s.tasks||[]).forEach(t=>{
      if(t.subItems){
        (t.subItems||[]).forEach(si=>{
          tot++;
          if(v.subChecks?.[si.id] || v.tasks?.['sub_na_'+si.id]==='na') done++;
        });
      } else {
        tot++;
        if(v.tasks?.[t.id]) done++;
      }
    }));
...
window.isAuditOverdue = window.isAuditOverdue || function(){
  const now = new Date();
  return now.getDate() > 10;
};

window.calcAuditPct = window.calcAuditPct || function(audit){
  if(!audit) return 0;
...
function renderHome(){
  ...
  list.innerHTML = S.houses.map(h=>{
    ...
    const noCount = audits.filter(a=>a&&hasNOs(a)).length;
    ...
    if(indivs.length>0){
      ...
      auditStatTxt = `Audits ${auditDone}/${indivs.length}`;
    }
    const noChip = hasNo ? `<span class="hhcard-chip chip-red">⚠ ${noCount} NO${noCount===1?'':'s'}</span>` : '';
