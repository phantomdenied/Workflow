...
function _drawSigFit(page, img, rect){
  ...
  // Fit signature to line width first, then cap height if needed.
  let drawW = boxW * 0.96;
  let drawH = dims.height * (drawW / Math.max(dims.width, 1));

  const maxH = boxH * 0.98;
  if(drawH > maxH){
    drawH = maxH;
    drawW = dims.width * (drawH / Math.max(dims.height, 1));
  }
