...
(function(){
  ...
  function getApiKey(){
    return (window.S && window.S.anthropicKey) ? String(window.S.anthropicKey).trim() : '';
  }
  ...
  function addSuggestedTasks(tasks){
    ...
    tasks.forEach(t => {
      ...
      window.S.rTasks.push({
        id: window.uid ? window.uid() : (Date.now() + Math.random().toString(36).slice(2,8)),
        name,
        freq: 'weekly',
        day: 1,
        time: 'morning',
  ...
  async function noteMakeTasks(){
    ...
    if(!txt){ toastSafe('Write or paste a note first'); return; }
    resultEl.classList.add('show');
    resultEl.textContent = 'Extracting tasks…';
    try{
      const data = await runJsonTask(
  ...
  async function emailMakeTasks(){
    ...
    resultEl.classList.add('show');
    resultEl.textContent = 'Reading email…';
    try{
      const data = await runJsonTask(
  ...
  function enhanceNotesUI(){
    ...
    if(notesScreen && !document.getElementById('aiEmailCard')){
  ...
  function enhanceFormTextareas(rootSel){
    ...
    root.querySelectorAll('textarea').forEach((ta) => {
      ...
      const [polishBtn, suggestBtn] = row.querySelectorAll('button');
      polishBtn.onclick = () => polishField(ta, 'polish');
      suggestBtn.onclick = () => polishField(ta, 'suggest');
